...
//    }
    ...
    public function readProductByBrandId(ReadProductByBrandId $request): JsonResponse
    {
        try {
            ...
            $request->validated();
            //todo action
            $products = Product::where('productBrandId', $request['productBrandId'])->get();
            if (!$products) throw new ExceptionUtil(ExceptionCase::UNABLE_TO_LOCATE_RECORD);
            foreach ($products as $product){
                $product->productImage;
            }
            return  $this->BASE_RESPONSE($products);
    ...
    public function readProductByCategoryId(ReadProductByCategoryId $request): JsonResponse
    {
        try {
            ...
            $request->validated();
            //todo action
            $products = Product::where('productCategoryId', $request['productCategoryId'])->get();
            if (!$products) throw new ExceptionUtil(ExceptionCase::UNABLE_TO_LOCATE_RECORD);
            foreach ($products as $product){
                $product->productImage;
            }
    ...
    public function filterProductBySellingPrice(FilterProductBySellingPrice $request): JsonResponse
    {
        try {
            ...
            $request->validated();
            //todo action
            $products = Product::where('productSellingPrice', '>=' ,$request['productMinSellingPrice'])
                                ->Where('productSellingPrice', '<=' ,$request['productMaxSellingPrice'])->get();
            if (!$products) throw new ExceptionUtil(ExceptionCase::UNABLE_TO_LOCATE_RECORD);
            foreach ($products as $product){
                $product->productImage;
            }
